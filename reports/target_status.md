...
| G4 强度进步 | **未达成** | 搜索显著优于启发式；**蒸馏模型仍与启发式无法区分**，见下 |
...
1. 夹具现在有区分度：搜索稳定胜过启发式（第 1 轮只有 0.0002 的噪声差）。
2. **蒸馏没有把搜索的优势搬进网络。** 第 3 轮把 model−heuristic 从 −0.001 提到 +0.009、
   区间从 ±0.08 收窄到 ±0.02，方向正确，**但统计上仍与启发式无法区分**。
   teacher top-1 一致率两轮都停在 0.72。这是最需要修的环节。
3. 纯网络的价值在**延迟**：3.4 ms vs 搜索 142 ms（≈42×），效用只差 0.027。
   hybrid 比纯搜索更慢（416 ms）却没换来更高效用 —— 当前预算下不划算。
4. **样本量仍是 smoke。** 按 `docs/00_ACCEPTANCE.md`，晋级需要 ≥1000 个固定验证场景的配对结果；
   `evaluate` 本身不给出晋级结论。本轮**不宣称 G4 达成**。
5. **已知配置问题**：数据从 2,374 涨到 6,795 条，但 `max_updates` 仍是 500
   （有效批次 32 → 每轮约 2.4 个 epoch）。下一轮应复制配置并提高 `max_updates`，
   而不是继续加数据。另外 32/64 局的评测重复使用了同一批 val 场景，按 `docs/00_ACCEPTANCE.md`
   这只能当开发信号，不能当最终结论。
